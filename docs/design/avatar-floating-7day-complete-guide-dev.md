...
2. 首页模板缺少 `static/tutorial/avatar/yui-standin.js` 时，YUI 头像演出运行时不完整。重启/重置后容易只剩教程流程状态，模型演出链路没有完整接上。
3. 教程临时模型通过 `window.handleModelReload(..., { temporaryConfig, skipIdleRestore, skipPersistentExpressions })` 切到 `yui-origin` 后，原先只恢复 DOM 容器和调用 `resumeRendering()`。普通角色切换路径在 `static/app-character.js` 会额外启动 PIXI ticker 并 `ticker.update()`；教程热切换路径缺少这一步时，就会出现容器、canvas、浮动按钮存在，但 Live2D 本体没有重新绘制。
4. 旧的教程 Live2D fade 准备态会向容器/canvas 写入 `opacity`、`visibility`、`transition`、`pointer-events` 等 inline style。reset、teardown 或异常结束时如果残留，会让后续模型加载成功但视觉仍不可见。
5. PC 端跳过按钮穿透属于 Electron Pet 输入层问题。`src/preload-pet.js` 的若干路径先判断 `point.overChatWindow === true`，再判断教程控件；外置聊天窗/教程 overlay 状态下，skip 按钮所在区域可能被归类为 overChatWindow，于是 Pet 窗口进入穿透，按钮收不到点击。
6. 2026-06-18 后续复现发现，问题不只发生在“重置 Day 1 后重启教程”：多次普通 reload、没有重新启动新手教程时，用户 Live2D 模型也可能视觉上消失。代码证据是首页初始加载链路 `static/live2d-init.js::initLive2DModel()` 在 `loadModel()` 成功后只更新 `window.LanLan1.live2dModel/currentModel` 并注册 unload cleanup，没有显式走 `showLive2d()`；而 `static/app-ui.js::showLive2d()` 原本只在 ticker stopped 时 `ticker.start()`，没有像热切换修复那样强制 currentModel/stage 可见、`ticker.update()` 和 `renderer.render(stage)`。因此普通 reload 不经过 `static/app-interpage.js::handleModelReload()` 时，吃不到 Live2D render activation，表现为按钮/位置状态可能存在，但 canvas 没有稳定绘制出模型。
...
2. 首页模板必须加载 `tutorial/avatar/yui-standin.js`，并保证它插入在 `tutorial/yui-guide/overlay.js` 之前、`tutorial/core/universal-manager.js` 之前；回归测试也按这个顺序约束，不能按旧描述挪到 overlay/page-handoff/performance stage 之后。`main_routers/pages_router.py` 的 static asset version 输入必须包含 `static/tutorial/avatar/yui-standin.js` 和 `static/app-interpage.js`，避免浏览器缓存旧 runtime。
...
9. PC 端教程控件必须在 `overChatWindow` 分支前做坐标命中判断。`isPointWithinTutorialInputSurface()` 命中 `#neko-tutorial-skip-btn`、`[data-yui-skip-control]`、`[data-yui-emergency-exit]`、`.yui-guide-overlay` 或 `.yui-guide-stage` 时，必须先 `setIgnoreState(false, true)` 并返回。这个判断要覆盖 `handleMousePosition()`、`updateDesktopAvatarToolCursorFromPoint()`、`syncMouseThroughStateWithCursor()` 和 `startMousePoller()`；不要再补回旧 Driver.js 的 `.driver-*` 输入面。
10. 不要用 MutationObserver 或教程开始事件 watcher 持续强制 PC 输入接管。最终保留的方案必须是局部坐标判断：只有鼠标实际位于可见教程输入面上时，Pet 窗口才强制不穿透。
...
22. 2026-06-19 追加复现：Day 2-7 的替身演出图片都不显示。明确代码证据是运行时页面加载的是旧的 `static/tutorial/yui-guide/avatar-standin.js`，它只包含残缺的 Day 2 旧 scene id；完整 cue 表实际在 `static/tutorial/avatar/yui-standin.js`，包含 Day 2-7 的 `day*_...` 新 scene id 和 `day5-character-settings` 资源。`static/tutorial/avatar/standin-controller.js` 只会读取 `window.YuiGuideAvatarStandIn.getCue(day, sceneId)`，所以加载旧表时 controller 查不到当前教程 scene，图片资源存在也不会渲染。修复契约是：首页、API 设置页和记忆页都必须在 `standin-controller.js` 前加载 `tutorial/avatar/yui-standin.js`，不得再加载旧 `tutorial/yui-guide/avatar-standin.js`；`main_routers/pages_router.py` 的 static asset version 输入也必须跟随 canonical cue 表，避免浏览器缓存旧表。
...
- PC 项目：`N.E.K.O.-PC/src/preload-pet.js`、`N.E.K.O.-PC/src/tutorial-global-overlay-service.js`、`N.E.K.O.-PC/test/main-composition-contract.test.js`
